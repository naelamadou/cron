var bindAjaxForm = function (selector, cb, beforeSerialize, beforeSend) {
  $(selector).each(function () {
    var self = this;
    var $self = $(this);
    var formcallback, bar, percent, status;
    $self.ajaxForm({
      dataType: 'json',
      type: 'POST',
      beforeSerialize: function (f, options) {
        beforeSerialize && beforeSerialize();
        formcallback = $(f).attr('callback');
      },
      ...
      beforeSend: function () {
        ...
        $('input[type=submit], button[type=submit]', $self).each(function () {
          var submitNode = $(this);
          submitNode.addClass('disabled loading');
          submitNode.attr('disabled', 'disabled');
        });
      ...
      uploadProgress: function (event, position, total, percentComplete) {
        ...
        if (percentComplete == 100) percentVal = '99%';
        bar.width(percentVal);
        percent.html('<strong>Chargement ' + percentVal + '</strong>');
      ...
      complete: function (xhr) {
        ...
        $('input[type=submit], button[type=submit]', $self).each(function () {
          var submitNode = $(this);
          submitNode.removeClass('disabled loading');
        ...
        if (xhr != null) {
          ...
          if (json && json != null) {
            ...
            if (json.success) {
              ...
              if (!cb && (!formcallback || formcallback == '') && json.redirect) {
